...
public class PersonBruteBehaviour : FriendlyBehaviour {
    [Header("Follow")]
    ...
    private void Start() {
        _properties.OnFallen.AddListener(ReleaseHostile);

        _nearestColliderDistance = _catchRange + 1;
    }

    private void Update() { // can be one liner with ? : maybe
        if (_properties.Standing && _nearestColliderDistance > _catchRange && GetNearestHostile()) _rb.velocity = _movementSpeed * Mathf.Sign(_nearestHostile.transform.position.x - transform.position.x) * Vector2.right;
