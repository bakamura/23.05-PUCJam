...
public class P_Animation : AnimationHandler {
    ...
    private void Update() {
        if (_animation[_currentAnimation].name != P_JUMP || (_animation[_currentAnimation].name == P_FALL && !P_Movement.Instance.PlayerGrounded() /* could be changed so groundCheck is only once per frame */)) {
